    ...
    <div className="mt-6 bg-healthcare-50 dark:bg-healthcare-900/20 rounded-xl border border-healthcare-200 dark:border-healthcare-800/50 p-5 shadow-sm animate-fadeInUp">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold font-display text-slate-800 dark:text-white">Smart Addendum Generator</h3>
        ...
        {!addendum && (
          <button
            ...
            disabled={loading}
            className="px-4 py-2 text-sm font-medium bg-healthcare-500 text-white rounded-xl hover:bg-healthcare-600 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 shadow-sm hover:shadow-md transition-all duration-200 hover:scale-105 active:scale-95"
          >
            {loading ? (
              <>
      ...
      {!addendum && !loading && (
        <div className="mb-3">
          ...
          <ul className="text-sm text-slate-600 dark:text-slate-400 space-y-1">
            ...
            {gaps.length > 5 && (
              <li className="text-slate-500 dark:text-slate-400">...and {gaps.length - 5} more</li>
            )}
          </ul>
        </div>
      )}
      ...
      {addendum && (
        <div className="space-y-4">
          ...
          {addendum.elementsAddressed?.length > 0 && (
            <div>
              <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-3 flex items-center gap-2">
                <svg className="w-4 h-4 text-healthcare-400" fill="currentColor" viewBox="0 0 20 20">
              ...
              <div className="flex flex-wrap gap-2">
                {addendum.elementsAddressed.map((element, idx) => (
                  <span
                    key={idx}
                    className="inline-flex items-center px-3 py-1.5 text-xs font-medium bg-healthcare-100 dark:bg-healthcare-900/30 text-healthcare-500 dark:text-trace rounded-full shadow-sm animate-scaleIn"
          ...
          <button
            onClick={handleGenerate}
            disabled={loading}
            className="text-sm text-healthcare-400 dark:text-trace-dim hover:text-healthcare-500 dark:hover:text-trace flex items-center gap-1"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
