...
const STATUS_ACCENT = {
  SUPPORTED: 'border-l-green-500',
  PARTIALLY_SUPPORTED: 'border-l-amber-500',
  NOT_SUPPORTED: 'border-l-red-500',
};

function CodeAnalysisCard({ analysis, index }) {
  const accent = STATUS_ACCENT[analysis.status] || 'border-l-amber-500';
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-4">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-12 h-12 rounded-xl bg-[#EDE6D3] dark:bg-instrument-bg-surface flex items-center justify-center shadow-[inset_0_1px_2px_rgba(0,0,0,0.06)] dark:shadow-[inset_0_1px_2px_rgba(0,0,0,0.15)] flex-shrink-0">
      ...
      {analysis.supportingElements?.length > 0 && (
        <div className="mb-4 p-3 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-100 dark:border-green-800/50 shadow-[inset_0_1px_2px_rgba(0,0,0,0.04)] dark:shadow-[inset_0_1px_2px_rgba(0,0,0,0.15)]">
      ...
      {analysis.missingElements?.length > 0 && (
        <div className="mb-4 p-3 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800/50 shadow-[inset_0_1px_2px_rgba(0,0,0,0.04)] dark:shadow-[inset_0_1px_2px_rgba(0,0,0,0.15)]">
      ...
      {analysis.fixSuggestions?.length > 0 && (
        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800/50 rounded-lg p-4 shadow-[inset_0_1px_2px_rgba(0,0,0,0.04)] dark:shadow-[inset_0_1px_2px_rgba(0,0,0,0.15)]">
...
export default function AnalysisReport({ report, note, selectedCptCodes, selectedPayer }) {
  ...
  const SCORE_COLORS = {
    HIGH: 'text-healthcare-500 dark:text-trace',
  ...
  return (
    <div className="space-y-6 print-friendly animate-fadeIn">
      <div className="bg-[#F5EFE0] dark:bg-instrument-bg-raised rounded-2xl border border-[#D6C9A8] dark:border-instrument-border shadow-card overflow-hidden relative">
        {/* Decorative gradient top accent */}
        <div className="h-1.5 bg-gradient-to-r from-healthcare-500 via-healthcare-400 to-trace"></div>

        {/* ── Hero Header: Score + Title ── */}
        <div className="px-4 sm:px-6 pt-6 pb-5">
          <div className="flex flex-col sm:flex-row items-center gap-5">
            {/* Score ring — hero size */}
            <div className="flex flex-col items-center flex-shrink-0">
              <ScoreRing score={report.overallScore} size={96} />
              <span className={`text-xs font-semibold mt-2 ${SCORE_COLORS[report.overallScore] || SCORE_COLORS.MEDIUM}`}>
                {SCORE_LABELS[report.overallScore] || 'Analysis Complete'}
              </span>
            </div>
        ...
        {report.emLevelRecommendation && (
          <div className="px-4 sm:px-6 pb-6 animate-fadeInUp" style={{ animationDelay: '100ms', opacity: 0 }}>
            <EMLevelCard
        ...
        {report.financialImpact && (
          <div className="px-4 sm:px-6 pb-6 animate-fadeInUp" style={{ animationDelay: '200ms', opacity: 0 }}>
            <FinancialImpact
        ...
        {/* ── Section: Payer-Specific Findings ── */}
        {report.payerSpecificFindings?.length > 0 && (
          <div className="px-4 sm:px-6 pb-6 animate-fadeInUp" style={{ animationDelay: '300ms', opacity: 0 }}>
        ...
        {/* ── Section divider — EKG blip ── */}
        <div className="mx-4 sm:mx-6 flex items-center gap-0">
          <div className="flex-1 border-t border-[#D6C9A8] dark:border-instrument-border"></div>
          <svg className="w-12 h-4 text-trace/40 dark:text-trace/30 flex-shrink-0 -my-px" viewBox="0 0 48 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
            <path d="M0,8 L14,8 L18,2 L22,14 L26,4 L30,8 L48,8" />
          </svg>
          <div className="flex-1 border-t border-[#D6C9A8] dark:border-instrument-border"></div>
        </div>
        ...
        <div className="px-4 sm:px-6 py-6 animate-fadeInUp" style={{ animationDelay: '500ms', opacity: 0 }}>
          <h3 className="text-[0.65rem] uppercase tracking-wide font-semibold text-slate-700 dark:text-slate-300 flex items-center gap-2 mb-4">
            ...
            <span className="ml-auto text-xs font-normal font-mono text-slate-500 dark:text-slate-400 bg-[#EDE6D3] dark:bg-instrument-bg-surface px-2 py-0.5 rounded-full">
              {report.codeAnalysis?.length || 0} codes
            </span>
          </h3>
        ...
        {/* ── Footer watermark ── */}
        <div className="border-t border-[#D6C9A8]/50 dark:border-instrument-border/50 px-4 sm:px-6 py-3 bg-[#EDE6D3]/50 dark:bg-instrument-bg-surface/30 no-print">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <StethoscopeIcon className="w-3.5 h-3.5 text-slate-400 dark:text-slate-500" />
              <span className="text-xs text-slate-400 dark:text-slate-500">
                DocDefend<span className="text-red-400 text-[10px] ml-px">✚</span>
              </span>
            </div>
            <span className="text-xs text-slate-400 dark:text-slate-500">
              Pre-claim QA &middot; AI-powered analysis
            </span>
          </div>
        </div>
      </div>
